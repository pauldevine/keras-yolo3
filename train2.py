...
def _main():
    ...
    if True:
        ...
        if USE_SGDAccum:
            parallel_model.compile(optimizer=SGDAccum(lr=MIN_LR, accum_iters=ACCUM_ITERS), loss={'yolo_loss': lambda y_true, y_pred: y_pred[0]}) # recompile to apply the change
        else:
            parallel_model.compile(optimizer=SGD(lr=MIN_LR), loss={'yolo_loss': lambda y_true, y_pred: y_pred[0]}) # recompile to apply the change
